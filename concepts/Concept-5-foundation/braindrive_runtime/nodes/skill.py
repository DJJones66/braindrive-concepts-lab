...
class SkillWorkflowNode(ProtocolNode):
    ...
    def _parse_manifest(self, manifest_path: Path) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            value_match = re.match(r"^\s{4}([A-Za-z0-9_.-]+)\s*:\s*(.+?)\s*$", line)
            if value_match:
                key = value_match.group(1).strip()
                value = value_match.group(2).strip().strip('"').strip("'")
                actions[current_action][key] = value
        ...
        if not actions:
            ...
            for action_name, meta in actions.items():
                operation = str(meta.get("operation", "")).strip()
                if not operation:
                    # Manifest-defined actions must declare operation explicitly.
                    return {}
                if "execution_tier" not in meta:
                    meta["execution_tier"] = "read"
    ...
    def _resolve_action(self, skill_id: str, action: str, parent_message_id: str | None) -> Tuple[Dict[str, Any] | None, Dict[str, Any] | None]:
        ...
        source = str(skill.get("source", "unknown")).strip()
        resolved = {
            "skill_id": skill_id,
            "action": action,
            ...
            "behavior": self._action_behavior(skill_id, action, action_meta, source),
        }
    ...
    @staticmethod
    def _normalize_interview_question(*, question: str, folder: str) -> str:
        ...
        if any(marker in lowered for marker in blocked_markers):
            return f"What is the primary software outcome you want from the '{folder}' workspace?"
        return cleaned
    ...
    def _build_next_question_prompt(self, *, folder: str, skill: str, answers: List[Dict[str, Any]]) -> str:
        ...
        return (
            f"{skill}\n\n"
            f"You are conducting a software requirements interview for BrainDrive workspace '{folder}'.\n"
            "In BrainDrive, a folder means a digital project workspace in the library.\n"
            "Never interpret folder as a physical/paper object and never ask physical-vs-digital questions.\n"
    ...
    def _build_completion_prompt(self, *, folder: str, skill: str, answers: List[Dict[str, Any]]) -> str:
        ...
        return (
            f"{skill}\n\n"
            f"Summarize this completed interview for BrainDrive workspace '{folder}'.\n"
    ...
    def _interview_start(
        ...
        *,
        ...
        question = self._normalize_interview_question(question=question, folder=folder)

        interview["asked_questions"] = [question]
        self._save_interview(folder, interview)
        self._save_skill_session(skill_id=skill_id, folder=folder, session=interview)
        self._save_skill_output(skill_id=skill_id, folder=folder, output={"question": question, "question_index": 0})

        question_intent = behavior.get("question_intent", "workflow.interview.question")
        return (
            question_intent,
            {
                "folder": folder,
    ...
    def _interview_continue(
        ...
        *,
        ...
        if len(answers) >= MIN_INTERVIEW_ANSWERS:
            next_intent = behavior.get("next_intent", "workflow.interview.complete")
            ready_intent = behavior.get("ready_intent", "workflow.interview.ready")
    ...
    def _interview_complete(
        ...
        *,
        ...
        completed_intent = behavior.get("completed_intent", "workflow.interview.completed")
        return (
            completed_intent,
            {
                "folder": folder,
    ...
    def _spec_generate(
        ...
        *,
        ...
        self._save_skill_output(skill_id=skill_id, folder=folder, output={"spec_markdown": generated})
    ...
    def _spec_propose_save(
        ...
        *,
        ...
        if not spec_markdown:
            ...
            if generated_intent != behavior.get("generated_intent", "workflow.spec.generated"):
        ...
        self._save_skill_output(skill_id=skill_id, folder=folder, output={"spec_markdown": spec_markdown})

        request_id = f"appr-{new_uuid()}"
        change = {
            "path": f"{folder}/{save_path}",
            "operation": "write",
    ...
    def _plan_generate(
        ...
        *,
        ...
        self._save_skill_output(skill_id=skill_id, folder=folder, output={"plan_markdown": generated})
        ...
        *,
        ...
        if not plan_markdown:
            ...
            if isinstance(generated, dict) and generated.get("intent") == "error":
                return generated
            generated_intent, generated_payload = generated
            if generated_intent != behavior.get("generated_intent", "workflow.plan.generated"):
        ...
        self._save_skill_output(skill_id=skill_id, folder=folder, output={"plan_markdown": plan_markdown})
        request_id = f"appr-{new_uuid()}"
        change = {
            "path": f"{folder}/{save_path}",
            "operation": "write",
    ...
    def _dispatch_action(
        ...
        *,
        ...
        behavior = resolved.get("behavior", {})
        if not isinstance(behavior, dict):
            behavior = {}
        operation = str(behavior.get("operation", "generic.execute")).strip() or "generic.execute"

        if operation in {"session.start", "session.step", "session.complete"}:
            if not folder:
        ...
        if operation in {"artifact.generate", "artifact.propose_save"}:
            if not folder:
                return make_error("E_NODE_ERROR", "No active folder selected", message.get("message_id"))

            artifact_kind = str(behavior.get("artifact_kind", "")).strip().lower()
            if artifact_kind == "spec":
                if operation == "artifact.generate":
            ...
            return make_error(
                "E_NODE_ERROR",
                f"Unsupported artifact_kind '{artifact_kind}' for operation '{operation}'",
                message.get("message_id"),
            )
