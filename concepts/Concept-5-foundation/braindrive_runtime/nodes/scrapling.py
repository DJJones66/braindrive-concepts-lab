...
class ScraplingNode(ProtocolNode):
    ...
    def _resolve_save_directory(self, raw_value: Any) -> str:
    ...
    def _write_via_memory_contract(
        self,
        *,
        relative_path: str,
        content: str,
        ...
        if self.ctx.route_message is None:
            raise RuntimeError("route_message is unavailable for memory contract write")

        write_response = self.ctx.route_message(
            {
                ...
                "extensions": {
                    "confirmation": {
                        "required": True,
                        "status": "approved",
                        "request_id": f"scrape-save-{parent_message_id or 'unknown'}",
                    }
                },
            }
        )
        if not isinstance(write_response, dict):
            raise RuntimeError("memory contract returned invalid response")
        if write_response.get("intent") == "error":
            error_payload = write_response.get("payload", {})
            error_obj = error_payload.get("error", {}) if isinstance(error_payload, dict) else {}
            code = str(error_obj.get("code", "E_NODE_ERROR"))
            message = str(error_obj.get("message", "memory write failed"))
            raise RuntimeError(f"{code}: {message}")
        if str(write_response.get("intent", "")) != "memory.write.applied":
            raise RuntimeError("memory write contract returned unexpected intent")

    def _save_results_via_memory_contract(
        self,
        *,
        results: List[Dict[str, Any]],
        extraction_type: str,
        ...
        relative_directory = self._resolve_save_directory(save_directory_raw)
        ...
        for index, item in enumerate(results):
            ...
            relative_path = f"{relative_directory}/{filename}" if relative_directory else filename
            self._write_via_memory_contract(
    ...
    def handle(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if save_to_library:
                storage = self._save_results_via_memory_contract(
                    results=trimmed,
                    extraction_type=extraction_type,
