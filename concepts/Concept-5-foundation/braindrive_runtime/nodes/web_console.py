...
class WebConsoleNode(ProtocolNode):
    ...
    def _prune_expired_sessions(self) -> int:
        ...
        for session_id, session in list(sessions.items()):
            ...
            sessions.pop(session_id, None)
            removed += 1
            self._log_session(session=session, event="session.closed", reason=reason or "timeout")
    ...
    def _health_summary(self) -> str:
        ...
        try:
            gateway = http_get_json(f"{self.gateway_base_url}/health", timeout_sec=self.http_timeout_sec)
            if isinstance(gateway, dict) and gateway.get("ok") is True:
                gateway_state = "ok"
        except Exception:
            gateway_state = "unreachable"
        ...
        return f"gateway: {gateway_state}\nrouter: {router_state}\nintent-router: {intent_state}\n"
    ...
    def _route_nl_command(self, *, session: Dict[str, Any], command: str, confirm: bool) -> str:
        ...
        try:
            gateway_result = http_post_json(
                f"{self.gateway_base_url}/api/v1/messages",
                {
                    "message": command,
            ...
            if isinstance(gateway_result, dict) and gateway_result.get("ok") is False:
                error_obj = gateway_result.get("error", {}) if isinstance(gateway_result.get("error", {}), dict) else {}
                raise RuntimeError(str(error_obj.get("message", "gateway returned error")))
            if isinstance(gateway_result, dict):
                conv_id = str(gateway_result.get("conversation_id", "")).strip()
                if conv_id:
                    session["conversation_id"] = conv_id
            result = gateway_result
        except Exception as exc:
            gateway_error = str(exc).strip() or type(exc).__name__
            if not self.allow_intent_fallback:
                return f"[E_NODE_UNAVAILABLE] gateway call failed: {gateway_error}"
            try:
                ...
                return (
                    f"[E_NODE_UNAVAILABLE] gateway call failed: {gateway_error}; "
                    f"intent-router fallback failed: {fallback_exc}"
                )

        if not isinstance(result, dict):
            return "[E_NODE_ERROR] invalid routing response"
